...
class Twig_Node_Module extends Twig_Node {
    ...
    protected function compileGetParent ( Twig_Compiler $compiler ) {
        ...
        if ( $this->getNode( 'parent' ) instanceof Twig_Node_Expression_Constant ) {
            ...
            $compiler->raw( "\$this->env->resolveTemplate(" )->subcompile( $this->getNode( 'parent' ) )->raw( ")" );
        }
    ...
    protected function compileConstructor ( Twig_Compiler $compiler ) {
        ...
        if ( null === $this->getNode( 'parent' ) ) {
            ...
            $compiler->write( "\$this->parent = \$this->env->loadTemplate(" )->subcompile( $this->getNode( 'parent' ) )->raw( ");\n\n" );
        }
        ...
        if ( $countTraits ) {
            ...
            foreach ( $this->getNode( 'traits' ) as $i => $trait ) {
                ...
                foreach ( $trait->getNode( 'targets' ) as $key => $value ) {
                    $compiler->write( sprintf( "\$_trait_%s_blocks[", $i ) )->subcompile( $value )->raw( sprintf( "] = \$_trait_%s_blocks[", $i ) )->string( $key )->raw( sprintf( "]; unset(\$_trait_%s_blocks[", $i ) )->string( $key )->raw( "]);\n\n" );
                }
            }
